...
export const loadMarkdownFiles = async (path: string) => {
  const storyPaths = glob.sync(`./md/${path}`);
  return Promise.all(
    storyPaths.map((storyPath) => {
      const modPath = storyPath.slice(storyPath.indexOf(`md/`) + 3);
...
export const loadStories = async (): Promise<PostData[]> => {
  return (await loadMarkdownFiles(`stories/*.md`))
    .map(mdToPost)
